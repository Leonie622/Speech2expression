...
parser.add_argument('--lambda1', type=float, default=0,
                    help='weight for instance-specific information in primal attribute')
parser.add_argument('--lambda2', type=float, default=0.75,
                    help='weight for instance-specific information in condition attribute')
...
parser.add_argument('--base', type=str, default='interfacegan',
                    help='use the attribute level direction solved by [interfacegan | ganspace]')
...
parser.add_argument('--real_image', type=int, choices=[0, 1], default=0,
                    help='edit real image or not, if true, you should specify the latent_code_path')
...
attr_level_dir_pri = torch.tensor(
    np.load(os.path.join(opt.pretrain_root, "attr_level_direction", opt.base, opt.dataset, "%s.npy" % opt.attr1)),
    dtype=torch.float).to(device)
attr_level_dir_cond = torch.tensor(
    np.load(os.path.join(opt.pretrain_root, "attr_level_direction", opt.base, opt.dataset, "%s.npy" % opt.attr2)),
    dtype=torch.float).to(device)
